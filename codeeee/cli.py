...
[bold yellow]TESTING[/bold yellow]
  test               Show interactive test menu (T1–T9)
  test T1            Connection + telemetry check  (no flight)
  test T2            Arm/disarm check              (no flight, motors spin)
  test T3            Takeoff + hover + land        (flies)
  test T4            Offboard velocity hover       (flies)
  test T5            Goto coordinate + RTL         (flies)
  test T6            Goto + loiter + RTL           (flies)
  test T7            Servo ground test             (no flight, props OFF)
  test T8            Goto + fire servo + RTL       (flies, DROPS payload)
  test T9            Full preflight checklist      (no flight)
  test all           Run all tests in sequence
  test results       Show previous test results
...
class CompanionCLI:
    ...
    def __init__(self, config: dict, state: SharedState,
        ...
        self.camera = camera
        self._log_lines = []
        self.test_suite = TestSuite(config, state, drone, servo)
    ...
    async def _dispatch(self, cmd: str):
        ...
        # Special case: 'test' takes an argument (e.g. 'test T3', 'test all')
        if base == "test":
            arg = parts[1].upper() if len(parts) > 1 else ""
            await self._cmd_test(arg)
            return
    ...
    async def _cmd_test(self, arg: str):
        """
        Run flight/system tests from the test suite.

        Usage:
          test           → show test menu (interactive)
          test T1        → connection check (no flight)
          test T2        → arm check (no flight)
          test T3        → takeoff + hover + land
          test T4        → offboard velocity hover
          test T5        → fly to coordinate + RTL
          test T6        → fly to coordinate + loiter + RTL
          test T7        → servo ground test (props OFF)
          test T8        → fly to coord + fire servo + RTL  ← full drop test
          test T9        → full preflight checklist
          test all       → run T1 through T9 in sequence
          test results   → show previous test results
        """
        if not arg or arg == "MENU":
            await self.test_suite.run_cli()
        elif arg == "RESULTS":
            self.test_suite._print_results()
        else:
            await self.test_suite.run(arg)
